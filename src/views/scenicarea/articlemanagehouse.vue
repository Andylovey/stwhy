<template>
  <div>
      ...
      <router-link :to="'/scenicarea/house/article/articlemanage/'+ pId + '/' + bId + '/add/addarticle'">
      ...
      height="calc(100% - 140px)">
       ...
       <el-table-column
        ...
        <template slot-scope="scope">
            <router-link :to="'/scenicarea/house/article/articlemanage/'+ pId + '/' + bId + '/detail/' + scope.row.ha_id">
                <el-button v-if="permissionDetail" type="text" size="small">查看</el-button>
            </router-link>
            <router-link :to="'/scenicarea/house/article/articlemanage/'+ pId + '/' + bId + '/edit/' + scope.row.ha_id">
...
export default {
  ...
  methods : {
      ...
      getInfo() {
          this.$http.get(`${api.dev}/p/main/houseArticle/show`,{params:{access_token:getToken(),id1:this.$route.params.bid,start:this.start,limit:this.limit}}).then(res => {
              if(res.body.code == 200) {
                  res.body.content.records.forEach(item => {
                      item.ha_date = formatDate(new Date(item.ha_date) , 'yyyy-MM-dd hh:mm:ss');
                  })
      ...
      handleDelete(row) {
          this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
            ...
            type: 'warning'
              ...
              this.loading = true;
              this.$http.post(`${api.dev}/p/main/houseArticle/delete`,{access_token : getToken(), ha_id: row.ha_id},{emulateJSON:true}).then(res => {
                  if(res.body.code == 200) {
                        this.$message({
                            message: '删除成功',
                            type: 'success'
