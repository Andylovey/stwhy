  ...
  <div>
    ...
    <el-dialog 
      ...
      <el-form ref="EditForm" :model="EditForm" label-width="80px" :rules="EditRules" style="width:100%;">
          <el-form-item label="名称" prop="p_name">
              <el-input v-model="EditForm.p_name"></el-input>
          </el-form-item>
          <el-form-item label="标识" prop="p_key">
              <el-input v-model="EditForm.p_key" readonly></el-input>
          </el-form-item>
          <el-form-item label="设备" prop="p_source">
                <el-select v-model="EditForm.p_source" placeholder="请选择" disabled>
                    <el-option key="0" label="PC端" value="0"></el-option>
                    <el-option key="1" label="移动端" value="1"></el-option>
                </el-select>
          </el-form-item>
          <el-form-item label="页面区域" prop="p_area">
              <el-input type="textarea" :rows="2" v-model="EditForm.p_area" readonly></el-input>
          </el-form-item>
    ...
    <el-dialog 
      ...
      <el-form ref="DetailForm" :model="DetailForm" label-width="80px" style="width:100%;">
          <el-form-item label="名称" prop="p_name">
              <el-input v-model="DetailForm.p_name" readonly></el-input>
          </el-form-item>
          <el-form-item label="标识" prop="p_key">
              <el-input v-model="DetailForm.p_key" readonly></el-input>
          </el-form-item>
          <el-form-item label="设备" prop="p_source">
                <el-select v-model="DetailForm.p_source" placeholder="请选择" disabled>
                    <el-option key="0" label="PC端" value="0"></el-option>
                    <el-option key="1" label="移动端" value="1"></el-option>
                </el-select>
          </el-form-item>
          <el-form-item label="页面区域" prop="p_area">
              <el-input type="textarea" :rows="2" v-model="DetailForm.p_area" readonly></el-input>
          </el-form-item>
      </el-form>
    </el-dialog>
    ...
    <el-dialog 
      ...
      <el-form ref="AddForm" :model="AddForm" label-width="80px" :rules="AddRules" style="width:100%;">
          ...
          <el-form-item label="标识" prop="p_key">
              <el-input v-model="AddForm.p_key"></el-input>
          </el-form-item>
          <el-form-item label="设备" prop="p_source">
                <el-select v-model="AddForm.p_source" placeholder="请选择" clearable>
                    <el-option key="0" label="PC端" value="0"></el-option>
                    <el-option key="1" label="移动端" value="1"></el-option>
                </el-select>
          </el-form-item>
...
export default {
  ...
  data() {
      return {
          ...
          EditForm : {},
          EditRules : {
            p_name : [{required: true,message : '请输入名称', trigger: 'blur'}],
            // p_key : [{required: true,message : '请输入标识', trigger: 'blur'}],
            // p_source : [{required: true,message : '请选择设备', trigger: 'blur'}],
          },
  ...
  methods : {
    ...
    getLimit() {
        ...
        handleDelete(row) {
          this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
            ...
            type: 'warning'
              ...
              this.loading = true;
              this.$http.post(`${api.dev}/p/main/page/delete`,{access_token : getToken() , p_id : row.p_id},{emulateJSON : true}).then(res => {
                    if(res.body.code == 200) {
                        this.$message({
                        message: '删除成功',
                        type: 'success'
      ...
      handleDetail(row) {
          ...
          this.$http.get(`${api.dev}/p/main/page/select`,{params:{access_token:getToken(),p_id:row.p_id}}).then(res => {
              if(res.body.code == 200) {
                    //loading.close();
                    //this.DetailDialogTableVisible = true;
                    this.DetailForm = res.body.content;
                    this.DetailForm.p_source = String(this.DetailForm.p_source);
                    this.$http.get(`${api.dev}/p/main/page/findPageArea`,{params:{access_token:getToken(),p_id:row.p_id}}).then(res => {
                        if(res.body.code == 200) {
                                var areaArr = []
                                res.body.content.forEach(item => {
                                    areaArr.push(item.pa_name)
                                })
                                var areaStr = areaArr.join(',');
                                this.DetailForm.p_area = areaStr;
      ...
      handleEdit(row) {
          ...
          this.$http.get(`${api.dev}/p/main/page/select`,{params:{access_token:getToken(),p_id:row.p_id}}).then(res => {
              if(res.body.code == 200) {       
                    ...
                    this.EditForm.p_source = String(this.EditForm.p_source);
                    this.$http.get(`${api.dev}/p/main/page/findPageArea`,{params:{access_token:getToken(),p_id:row.p_id}}).then(res => {
                        if(res.body.code == 200) {
                                var areaArr = []
                                res.body.content.forEach(item => {
          ...
          this.$refs[formName].validate((valid) => {
            if(valid) {
                // 开启加载器
                    ...
                    formdata.append('p_id',this.EditForm.p_id);
                    formdata.append('p_name',this.EditForm.p_name);
                    formdata.append('p_key',this.EditForm.p_key);
                    formdata.append('p_source',this.EditForm.p_source);
                    this.$http.post(`${api.dev}/p/main/page/update`,formdata,{emulateJSON:true}).then(res => {
                        if(res.body.code == 200) {
                            loading.close();
                            this.EditDialogTableVisible = false;
                            this.$message({
      ...
      submitAddForm(formName) {
          this.$refs[formName].validate((valid) => {
              if(valid) {
                    ...
                    var formdata = new FormData();
                    formdata.append('access_token',getToken());
                    formdata.append('p_name',this.AddForm.p_name);
                    formdata.append('p_key',this.AddForm.p_key);
                    formdata.append('p_source',this.AddForm.p_source);
                    this.$http.post(`${api.dev}/p/main/page/save`,formdata,{emulateJSON:true}).then(res => {
                        if(res.body.code == 200) {
                            loading.close();
                            this.AddDialogTableVisible = false;
                            this.$message({
