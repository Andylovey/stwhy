...
//   components: { treeTable },
  ...
  methods: {
    ...
    getTopic() {
        this.$http.get(`${api.dev}/p/main/activity/findTopicWithPermission`,{params:{access_token: getToken()}}).then(res => {
            if(res.body.code == 200) {
    ...
    getFirstTopic() {
        this.$http.get(`${api.dev}/p/main/activity/findCategoryWithPermission`,{params: {access_token: getToken(), c_topic : this.topicId}}).then(res => {
            if(res.body.code == 200) {
                ...
                this.$http.get(`${api.dev}/p/main/activity/show`,{params: {access_token : getToken() , categoryId:this.categoryId , start:this.start , limit : this.limit}}).then(res => {
                    if(res.body.code == 200) {
                        // 先过滤
        ...
        this.$http.get(`${api.dev}/p/main/category/select`,{params: {access_token:getToken(),c_id : this.AddCategoryId}}).then(res => {
            if(res.body.code == 200) {
                if(res.body.content.category.c_single == 0 && res.body.content.category.c_article != '') {
                    ...
                    this.$http.get(`${api.dev}/p/main/activity/select`,{params: {access_token : getToken(),a_id : res.body.content.category.c_article,c_id:this.categoryId}}).then(res => {
                        if(res.body.code == 200) {
                            loading.close();
    ...
    handleDetail(row) {
          ...
          this.$http.get(`${api.dev}/p/main/activity/select`,{params: {access_token: getToken(),a_id:row.a_id,c_id:this.categoryId}}).then(res => {
              if(res.body.code == 200) {
                loading.close();
    ...
    handleDelete(row) {
        this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
            ...
            this.loading = true;
            this.$http.post(`${api.dev}/p/main/activity/delete`,{access_token:getToken(),a_id:row.a_id},{emulateJSON: true}).then(res => {
                if(res.body.code == 200) {
                    this.$message({
                        message: '删除成功',
                        type: 'success'
    ...
    submitEditForm(formName) {
        this.$refs[formName].validate((valid) => {
            if(valid){
                ...
                this.$http.post(`${api.dev}/p/main/activity/update`,formdata,{emulateJSON:true}).then(res => {
                    if(res.body.code == 200) {
                        loading.close();
    ...
    submitAddForm(formName) {
        this.$refs[formName].validate((valid) => {
            if(valid) {
                ...
                this.$http.post(`${api.dev}/p/main/activity/save`,formdata,{emulateJSON:true}).then(res => {
                    if(res.body.code == 200) {
                        loading.close();
