<template>
  <div>
    ...
    <router-link :to="'/scenicarea/village/villageadd/add'">
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <router-link :to="'/scenicarea/village/villagedetail/'+scope.row.b_id">
                <el-button v-if="permissionDetail" type="text" size="small">查看</el-button>
            </router-link>
            <router-link :to="'/scenicarea/village/villageedit/'+scope.row.b_id">
    ...
    <el-dialog 
      ...
      <el-form ref="AddForm" :model="AddForm" label-width="80px" :rules="AddRules">
        ...
        <el-form-item label="视频上传" prop="video">
            <el-upload
              class="upload-demo"
              :action="uploadAddVideo"
              :on-preview="AddHandlePreview"
              :on-success="handleAddVideoSuccess"
              :before-upload="beforeAddUploadVideo"
              ref="AddVideoUpload"
              list-type="picture">
              <el-button size="small" type="primary">点击上传</el-button>
            </el-upload>
            <el-dialog :visible.sync="AddDialogVisible" :modal="false" @closed="handleAddVideoClose('video')">
              <video :src="AddForm.video" controls="controls" width="100%" ref="video"></video>
            </el-dialog>
        </el-form-item>
        <el-form-item label="序号" prop="b_id">
...
export default {
  ...
  data(){
    return {
      ...
      uploadAddVideo : `${api.dev}/p/main/upload/video`, // 上传视频的地址
      AddDialogVisible : false, // 放大出现视频
  ...
  methods : {
    ...
    getInfo(){
      this.$http.get(`${api.dev}/p/main/village/show`,{params : {access_token: getToken() ,start : this.start,limit : this.limit}}).then(res => {
        if(res.body.code == 200) {
    ...
    handleDetail(row) {
      ...
      this.$http.get(`${api.dev}/p/main/village/select`,{params: {access_token : getToken() , b_id : row.b_id}}).then(res => {
        if(res.body.code == 200) {
          // 关闭加载器
          loading.close();
          ...
          this.DetailForm.b_photo = res.body.content.village.b_photo;
    ...
    handleEdit(row) {
      ...
      this.$http.get(`${api.dev}/p/main/village/select`,{params: {access_token : getToken() , b_id : row.b_id}}).then(res => {
        if(res.body.code == 200) {
          ...
          this.EditForm.b_photo = res.body.content.village.b_photo;
          ...
          this.EditFileList = [{url: res.body.content.village.b_photo}];
        }else if (res.body.code == 500) {
            this.$message({
              message: res.body.msg,
              type: 'error'
    ...
    handleDelete(row){
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        this.loading = true;
        this.$http.post(`${api.dev}/p/main/village/delete`,{access_token: getToken(),b_id:row.b_id},{emulateJSON:true}).then(res => {
          if(res.body.code == 200) {
            this.$message({
              message: '删除成功',
              type: 'success'
    ...
    cancleAddForm(formName) {
      ...
      this.$refs.AddUpload.clearFiles(); // 清除图片位置
      this.$refs.AddVideoUpload.clearFiles(); // 清除视频位置
    },
    ...
    // 上传视频过程中
    // AddVideoProgress(event,file,fileList){
    //   const loading = this.$loading({
    //       lock: true,
    //       text: 'Loading',
    //       spinner: 'el-icon-loading',
    ...
    handleAddVideoSuccess(res,file) {
      if(res.code == 200){
        this.AddForm.video = `${api.address}${res.content}`;
      }else {
        this.$message({
    ...
    AddHandlePreview(file) {
      ...
      this.AddForm.video = file.url;
    },
    // 关闭视频模态框
    handleAddVideoClose(formName){
      this.$refs[formName].pause();
    },
    // 分页
