    ...
    AddSave(formName) {
        this.$refs[formName].validate((valid) => {
            if(valid) {
                ...
                // }
                for(var i = 0 ; i < this.AddForm.fields.length ; i++) {
                    if(this.AddForm.fields[i].f_type == 4) {
                        formdata.append(this.AddForm.fields[i].f_key,this.videoPlaySource)
                    }
                    ...
                    // if(this.AddForm.fields[i].f_value != null && this.AddForm.fields[i].f_type != 7) {
                        if(this.AddForm.fields[i].f_type != 7 && this.AddForm.fields[i].f_type != 8 && this.AddForm.fields[i].f_type != 4) {
                            formdata.append(this.AddForm.fields[i].f_key,this.AddForm.fields[i].f_value);
                        }    
                    // }    
                }
    ...
    handleDeleteVideoArr(item) {
        this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          type: 'warning'
            ...
            this.AddForm.fields.forEach(items => {
                if(items.f_type == 8) {
