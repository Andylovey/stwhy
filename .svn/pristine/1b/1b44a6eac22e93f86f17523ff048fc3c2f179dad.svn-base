  ...
  <div class="app-container">
    <el-tabs :tab-position="tabPosition" @tab-click="handleClickTopic" type="border-card" style="height:100%;">
        <el-tab-pane v-for="item in topicMsg" :key="item.t_id">
                ...
                row-style="height:40px">
                    ...
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            ...
                            <router-link :to="'/informationman/information/informationdetail/'+scope.row.a_id + '/' + categoryId">
                                <el-button v-if="permissionDetail" type="text" size="small">查看</el-button>
                            </router-link>
                            <router-link :to="'/informationman/information/informationedit/'+scope.row.a_id + '/' + categoryId">
            ...
            <!-- 分页 -->
            <div id="pagination" style="position:relative;bottom:-10px;">
            <el-pagination
                background
    ...
    getFirstTopic() {
        this.$http.get(`${api.dev}/p/main/information/findCategoryWithPermission`,{params: {access_token: getToken(), c_topic : this.topicId}}).then(res => {
            if(res.body.code == 200) {
                ...
                this.$http.get(`${api.dev}/p/main/information/show`,{params: {access_token : getToken() , categoryId:this.categoryId , start:this.start , limit : this.limit}}).then(res => {
                    if(res.body.code == 200) {
                        // 先过滤
        ...
        this.$http.get(`${api.dev}/p/main/category/select`,{params: {access_token:getToken(),c_id : this.AddCategoryId}}).then(res => {
            if(res.body.code == 200) {
                if(res.body.content.category.c_single == 0 && res.body.content.category.c_article) {
                    ...
                    this.$router.push({
                        path : '/informationman/information/informationadd/' + this.AddCategoryId
                    })
                }
            }else if(res.body.code == 500) {
                this.$message({
